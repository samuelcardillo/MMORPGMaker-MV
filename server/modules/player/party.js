...
exports.initialize = function() {
  ...
  io.on("connect", function(player) {
    ...
    player.on("party_player_join_fight", async function(gameTroop) {
      ...
      if(exports.combat[player.isInParty][player.playerData.mapId]) {
        ...
        if(exports.combat[player.isInParty][player.playerData.mapId].members[player.id]) return  exports.combat[player.isInParty][player.playerData.mapId].state = gameTroop;
      ...
      let partyMembers = await MMO_Core["socket"].modules["player"].subs["player"].getPlayers(player.isInParty) || {};

      // We iterate through to them to invoke them in the battle if on the same map
      for(var k in partyMembers) {
        if(partyMembers[k].id === player.id) continue; // If member is the same than player, we do nothing.
        if(partyMembers[k].playerData.mapId !== player.playerData.mapId) continue; // If member is not on the same map, we don't nothing.
    ...
    player.on("party_player_action_fight", async (action) => { 
      if(!player.isInParty || !exports.combat[player.isInParty] || !exports.combat[player.isInParty][player.playerData.mapId]) return;
      ...
      let actionsLength = Object.keys(exports.combat[player.isInParty][player.playerData.mapId].actions).length;
      let membersLength = Object.keys(exports.combat[player.isInParty][player.playerData.mapId].members).length;
      
      // If all players have finished doing actions
      if(actionsLength === membersLength) {
        let actions =  exports.combat[player.isInParty][player.playerData.mapId].actions;
    ...
    player.on("party_player_end_fight", function() {
      if(!player.isInParty);
      if(!exports.combat[player.isInParty]) return;
      if(!exports.combat[player.isInParty][player.playerData.mapId]) return;

      player.playerData.isInCombat = false;      

      if(exports.combat[player.isInParty][player.playerData.mapId].initiator.id === player.id) exports.disbandFight(player.isInParty, player.playerData.mapId);
    })
    ...
    player.on("disconnect", () => {
      ...
      if(exports.combat[player.isInParty]) { 
        let mapId = player.playerData.mapId;
        if(exports.combat[player.isInParty][mapId]) {
          if(exports.combat[player.isInParty][mapId].members[player.id]) delete exports.combat[player.isInParty][mapId].members[player.id];
          if(exports.combat[player.isInParty][mapId].initiator["id"] === player.id) exports.disbandFight(player.isInParty, mapId);
        }        
      }
  ...
  exports.getPartyLeader = async partyName => {
    let hostId = partyName.split("party-")[1];
  ...
  exports.leaveParty = async leaver => {
    if(!leaver.isInParty) return;

    let partyName = leaver.isInParty;
    let partyLeader = await exports.getPartyLeader(leaver.isInParty);
    ...
    leaver.leave(partyName, () => {
      ...
      if(partyLeader === undefined || leaver.id === partyLeader.id) return exports.disbandParty(partyName);
    });
    
  }

  exports.disbandParty = async partyName => {    
    let partyMembers = await MMO_Core["socket"].getConnectedSockets(partyName);
    let partyLeader  = await MMO_Core["socket"].modules["player"].subs["player"].getPlayerById(partyName.split("party-")[1]);
  ...
  exports.refreshPartyData = async partyName => {
    if(!partyName || io.sockets.adapter.rooms[partyName] === undefined) return;

    let rawPartyMembers = await MMO_Core["socket"].modules["player"].subs["player"].getPlayers(partyName) || {};
    let partyMembers = {};

    for(var k in rawPartyMembers) {
      partyMembers[k] = rawPartyMembers[k].playerData;
      partyMembers[k].isInitiator = (exports.combat[partyName][rawPartyMembers[k].playerData.mapId] && exports.combat[partyName][rawPartyMembers[k].playerData.mapId].initiator.id === rawPartyMembers[k].id) ? true : false;
      partyMembers[k].isInCombat = (exports.combat[partyName][rawPartyMembers[k].playerData.mapId] && exports.combat[partyName][rawPartyMembers[k].playerData.mapId].members[rawPartyMembers[k].id]) ? true : false;
    }
  ...
  exports.isInCombat = async player => {
    if(!player.isInParty || !exports.combat[player.isInParty][player.playerData.mapId]) return false;
    if(exports.combat[player.isInParty][player.playerData.mapId].initiator.id === player.id) return false;

    return exports.combat[player.isInParty][player.playerData.mapId];
  }

  exports.joinFight = player => {
    if(exports.combat[player.isInParty][player.playerData.mapId].members[player.id]) return;
  ...
  exports.disbandFight = async (partyName, mapId) => {
    if(!exports.combat[partyName][mapId]) return;

    let party  = exports.combat[partyName][mapId];
    delete exports.combat[partyName][mapId];

    let rawPartyMembers = await MMO_Core["socket"].modules["player"].subs["player"].getPlayers(partyName) || {};

    for(var k in rawPartyMembers) {
      if(rawPartyMembers[k].playerData.mapId !== party.initiator.playerData.mapId) continue;
