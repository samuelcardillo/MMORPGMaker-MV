exports.initialize = function(io) {
  exports.use = function(args, initiator) {
    if(args.length < 3) return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", "Not enough arguments.", "error");    
    ...
    MMO_Core["database"].findUser({username: initiator.playerData["username"]}, function(output) {
      if(oldPassword !== output[0]["password"]) return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", "Wrong old password.", "error");    
