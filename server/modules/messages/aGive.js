exports.initialize = function () {
  ...
  exports.use = async function (args, initiator) {
    ...
    let player = {};
    let amount = parseInt((args[4] !== undefined) ? args[4] : args[3]);
    let itemId = parseInt(args[3]);
    ...
    if (isNaN(args[3])) return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", `Value is not valid.`, "error");
    if (args.length < 4) return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", "Not enough arguments.", "error");
    ...
    if (args[2] === "gold") {
      ...
      MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", `You gave ${args[3]} gold to ${players[args[1]].playerData.username}!`, "action");
        ...
        if (isNaN(itemId)) return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", `Value is not valid.`, "error");
        if (itemId < 1) return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", `${players[args[1]].playerData.username} already has this skill.`, "error");
      ...
      if (initiator.playerData["permission"] < amount) return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", `You don't have the permission to give that amount of permission!`, "error");
      ...
      if (args[2] !== "weapons" && args[2] !== "items" && args[2] !== "armors") return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", `Item type is not valid.`, "error");
      if (isNaN(args[4])) return MMO_Core["socket"].modules["messages"].sendToPlayer(initiator, "System", `Value is not valid.`, "error");

      if (players[args[1]].playerData.stats[args[2]][itemId]) players[args[1]].playerData.stats[args[2]][itemId] += amount;
      else players[args[1]].playerData.stats[args[2]][itemId] = amount;
