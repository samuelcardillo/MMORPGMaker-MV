...
(function() {
    ...
    // If it is a party battle and the player selected an action
        if ($gameParty.size() > 1 && BattleManager.actor().index() > 0) {
            if (MMO_Core_Player.isCombatInitiator) {
                BattleManager._canEscape = true;
            } // If combat initiator, we can escape the combat.

            BattleManager._actorIndex = 0; // We make sure he can't control the others
            const action = BattleManager.actor()._actions[0]; // We store its action
    ...
    Scene_Battle.prototype.updateStatusWindow = function() {
        if (this.isActive() && !this._messageWindow.isClosing()) {
            if (MMO_Core_Party.Party.length > 0) {
                for (let i = 0; i < SceneManager._scene._spriteset.children[0].children[2].children.length; i++) {
                    if (!(SceneManager._scene._spriteset.children[0].children[2].children[i] instanceof Sprite_Actor)) {
                        continue;
                    }
                    if (SceneManager._scene._spriteset.children[0].children[2].children[i]._actor) {
                        continue;
                    }
    ...
    Game_Unit.prototype.onBattleEnd = function() {
        if (this instanceof Game_Party && $gameParty.size() > 1) {
            MMO_Core.socket.emit("party_player_end_fight");
        }
    ...
    BattleManager.endBattle = function(result) {
        if ($gameParty.size() > 1 && !MMO_Core_Player.isCombatInitiator) {
            return;
        } // We block it.
    ...
    MMO_Core.socket.on("party_player_join_fight", function(payload) {
        if ($gameParty._inBattle || !payload || !payload._troopId) {
            return;
        }

        BattleManager.setup(payload._troopId, false, true);

        for (let i = 0; i < $gameTroop._enemies.length; i++) {
    ...
    MMO_Core.socket.on("party_player_estimate_fight", async function(actions) {
        const battleMembers = BattleManager.allBattleMembers();
        const results = {};

        // We go through every battle members to classify them.
        for (let i = 0; i < battleMembers.length; i++) {
            if (battleMembers[i]._actions[0] === undefined) {
                i++; continue;
            }

            // If Game Actor then we set up the actions.
            if (battleMembers[i] instanceof Game_Actor) {
                const action = actions[battleMembers[i]._name]; // We assign the action corresponding to the player
            ...
            const actorName = battleMembers[i].name();
            const targets = battleMembers[i]._actions[0].makeTargets();
            results[actorName] = {};
            ...
            for (const k in targets) {
                const targetName = targets[k].name();
    ...
    MMO_Core.socket.on("party_player_action_fight", async function(results) {
        ...
        for (const actorName in actors) {
            const result = results[actorName];
            const currentActor = actors[actorName];
            ...
            if (result) {
                for (const targetName in result.targets) {
            ...
            if (action) {
                ...
                if (action.isValid()) {
                    ...
                    // updateAction()
                    for (const k in BattleManager._targets) {
                        const currentTarget = BattleManager._targets.shift();
                        if (currentTarget) {
                            // invokeAction()
