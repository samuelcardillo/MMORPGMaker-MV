...
(function() {
    ...
    MMO_Core.socket.on("map_joined", function(data) {
        ...
        if (MMO_Core_Party.Party.length > 0 && !MMO_Core_Player.Player.isInCombat) {
            for (let i = 0; i < MMO_Core_Party.Party.length; i++) {
                if (MMO_Core_Party.Party[i].username != data.playerData.username) {
                    continue;
                }
                if (!MMO_Core_Party.Party[i].isInCombat) {
                    continue;
                }
    ...
    MMO_Core.socket.on("map_exited", function(data) {
        if (MMO_Core_Players.Players[data] === undefined) {
            return;
        }
        if ($gameMap._events[MMO_Core_Players.Players[data]._eventId] === undefined) {
            return;
        }
    ...
    MMO_Core.socket.on("refresh_players_on_map", function(payload) {
        if (MMO_Core_Players.Players[payload.playerId] === undefined) {
            return;
        }
    ...
    MMO_Core.socket.on("player_moving", function(data) {
        ...
        if (MMO_Core_Players.Players[data.id] === undefined) {
            return;
        }
