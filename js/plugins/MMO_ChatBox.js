...
(function() {
    ...
    window.addEventListener("resize", function() {
        if (!ChatBox.isGenerated || !ChatBox.isVisible) {
            return;
        }
    ...
    ChatBox.toggle = function() {
        if (document.querySelector("#chatbox_box") === null) {
            return;
        }

        const state = (this.isVisible) ? "hidden" : "visible";
        const chatboxInput = document.querySelector("#chatbox_input");
        const chatboxBox = document.querySelector("#chatbox_box");
    ...
    function sendMessage(e) {
        ...
        if (e.keyCode === undefined && e !== "touch") {
            return;
        }

        const message = document.querySelector("#chatbox_input").value;
        if (message.length <= 0) {
            return;
        }
    ...
    MMO_Core.socket.on("new_message", function(messageData) {
        ...
        const message = document.createTextNode(messageData.username + ": " + messageData.msg);
