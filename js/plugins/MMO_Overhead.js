...
MMO_Overhead.Windows.prototype.bufferX = function() {
    if (this._bufferX !== undefined) {
        return this._bufferX;
    }
    return 0;
};

MMO_Overhead.Windows.prototype.bufferY = function() {
    if (this._bufferY !== undefined) {
        return this._bufferY;
    }
    return 36;
};
...
MMO_Overhead.Windows.prototype.gatherDisplayData = function() {
    ...
    const list = this._character.list();
    const max = list.length;
    let comment = "";
    for (let i = 0; i < max; ++i) {
        const ev = list[i];
        if ([108, 408].contains(ev.code)) {
            comment += ev.parameters[0] + "\n";
        }
    }
...
MMO_Overhead.Windows.prototype.refresh = function() {
    this.contents.clear();
    let txWidth = this.textWidthEx(this._text);
    ...
    this.createContents();
    const wx = (this.contents.width - txWidth) / 2;
    const wy = 0;
...
MMO_Overhead.Windows.prototype.updatePage = function() {
    if (this._pageIndex === this._character._pageIndex) {
        return;
    }
    this._pageIndex = this._character._pageIndex;
    this.contents.clear();
...
Sprite_Character.prototype.refreshMiniLabel = function() {
    if (this._miniLabel) {
        this._miniLabel.forceRefresh();
    }
};

Scene_Map.prototype.refreshAllMiniLabels = function() {
    const length = this._spriteset._characterSprites.length;
    for (let i = 0; i < length; ++i) {
        const sp = this._spriteset._characterSprites[i];

        // If there is no minilabel then we do nothing
        if (sp._miniLabel === undefined) {
            return;
        }

        if (sp._character._isBusy && sp._miniLabel._text.length > 0) {
            sp._miniLabel._text = `(${sp._character._isBusy}) ${sp._miniLabel._initialText}`;
        }
        if (!sp._character._isBusy && sp._miniLabel._text.length > 0) {
            sp._miniLabel._text = `${sp._miniLabel._initialText}`;
        }

        // If player is in party
        if (MMO_Core_Party.Party.length > 0) {
            for (let c = 0; c < MMO_Core_Party.Party.length; c++) {
                if (!MMO_Core_Party.Party[c]) {
                    continue;
                }
                // If player is in the same party than us then we write his name in blue
                if (MMO_Core_Party.Party[c].username === sp._miniLabel._initialText) {
                    sp._miniLabel._text = "\\C[23]" + sp._miniLabel._text;
                }
            }
        }
