...
(function() {
    ...
    SceneManager.changeScene = function() {
        if (this.isSceneChanging() && !this.isCurrentSceneBusy()) {
            if (SceneManager._nextScene instanceof Scene_Menu) {
                if (!MMO_Core_Player.Player.isOnMenu) {
                    MMO_Core_Player.updateBusy("menu");
                }
    ...
    Game_Actor.prototype.setup = function(actorId) {
        ...
        const playerName = MMO_Core_Player.Player.username;
    ...
    Scene_Map.prototype.onMapLoaded = function() {
        ...
        if (MMO_Core_Player.Player.skin.characterIndex === undefined) {
            MMO_Core_Player.Player.skin.characterIndex = 1;
        }
    ...
    Game_Player.prototype.moveByInput = function() {
        if (!this.isMoving() && this.canMove()) {
            ...
            if (direction > 0) {
                ...
                if (MMO_Core_Player.Parameters["Mouse Movements"] === "false") {
    ...
    MMO_Core_Player.refreshStats = async function() {
        ...
        if (MMO_Core_Party) {
            await MMO_Core_Party.refreshPartyStats();
        }
    ...
    MMO_Core_Player.savePlayerStats = function() {
        if ($gameActors._data[1] === undefined) {
            return;
        }

        const equips = [];
        for (let i = 0; i < $gameActors._data[1]._equips.length; i++) {
