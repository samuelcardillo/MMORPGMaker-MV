...
(function() {
    ...
    MMO_Core.socket.on("refresh_party_data", async function(payload) {
        if (payload[MMO_Core_Player.Player.username]) {
            MMO_Core_Player.isCombatInitiator = !!(payload[MMO_Core_Player.Player.username].isInitiator);
            MMO_Core_Player.Player.isInCombat = !!(payload[MMO_Core_Player.Player.username].isInCombat);
        }

        delete payload[MMO_Core_Player.Player.username];

        const partySize = Object.keys(payload).length; // If no one = 1
        const currentPartySize = $gameParty.size(); // If no one = 1
        ...
        if (partySize > 0) {
            for (var i = 0; i <= partySize; i++) {
                if (i === partySize) {
                    return MMO_Core_Player.refreshStats();
                }

                const playerName = Object.keys(payload)[i];
    ...
    MMO_Core_Party.refreshPartyStats = async function() {
        ...
        for (let i = 0; i < MMO_Core_Party.Party.length; i++) {
            if (!$gameActors._data[i + 2]) {
                continue;
            }
